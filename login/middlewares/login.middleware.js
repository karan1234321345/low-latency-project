...
export function loginMidleware(req,reply,done) {
    try {
        if (!checkBody(req)) {
            return insufficientDataResponse(reply);
        };
        const {userId,password,deviceFingerPrint} = req.body;
        const sanatizedData = {userId:sanatizeInput(userId),password:password.trim(),deviceFingerPrint};
        if (!sanatizedData.password ||!sanatizedData.userId ||!sanatizedData.deviceFingerPrint) {
            return insufficientDataResponse(reply);
        };
        req.body = sanatizedData;
        return done();
    } catch (error) {
        console.log("error in the middleware function of the login middleware",error.message);
